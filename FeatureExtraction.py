...
def blink_detector(output_file, input_video):
    ...
    while True:
        ...
        if (np.size(rects) != 0):
            ...
            if 2*dist.euclidean(shape[12], shape[54]) < dist.euclidean(shape[4], shape[48]):

                ear = leftEAR

            elif 2*dist.euclidean(shape[4], shape[48]) <  dist.euclidean(shape[12], shape[54]):
...
blink_detector("DeepLearning-DrowsinessDetection/Dataset/alert.txt", "DeepLearning-DrowsinessDetection\Dataset\mypaperdataset.mp4")
